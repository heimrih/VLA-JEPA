...
class TrainerUtils:
                ...
                try:
                    ...
                    if sub_state_dict:
                        ...
                        compatible_state_dict = {
                            ...
                            if k in module_state_dict and module_state_dict[k].shape == v.shape
                        }
                        skipped_keys = sorted(set(sub_state_dict) - set(compatible_state_dict))
                        module.load_state_dict(compatible_state_dict, strict=False)
                        if dist.get_rank() == 0:
                            print(f"✅ parameters loaded to module '{path}' ({len(compatible_state_dict)}/{len(sub_state_dict)} tensors)")
                            if skipped_keys:
                                print(f"⚠️ skipped {len(skipped_keys)} incompatible tensors in '{path}', examples: {skipped_keys[:10]}")
    ...
    def _reset_dataloader(dataloader, epoch_counter):
        ...
        # 2. set new epoch (Accelerate DataLoaderShard or distributed sampler)
