...
def make_unique_run_id(base_run_id: str) -> str:
    """create a run id that does not overwrite previous checkpoint folders"""
    job_id = os.environ.get("SLURM_JOB_ID")
    array_task_id = os.environ.get("SLURM_ARRAY_TASK_ID")
    if job_id and array_task_id:
        return f"{base_run_id}-{job_id}_{array_task_id}"
    if job_id:
        return f"{base_run_id}-{job_id}"
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    return f"{base_run_id}-{timestamp}"


def setup_directories(cfg) -> Path:
    """create output directory and save config"""
    is_resume = bool(OmegaConf.select(cfg, "trainer.is_resume", default=False))
    if not is_resume:
        cfg.run_id = make_unique_run_id(str(cfg.run_id))
