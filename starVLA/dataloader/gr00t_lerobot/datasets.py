...
LE_ROBOT_STANDARD_STATS_FILENAME = "meta/stats.json"
...
def _atomic_write_pickle(path: Path, data: dict) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
...
def calculate_dataset_statistics(parquet_paths: list[Path]) -> dict:
    ...
    for parquet_path in tqdm(
        sorted(path for path in parquet_paths if not path.name.startswith("._")),
    ...
    for le_modality in all_low_dim_data.columns:
        ...
        try:
            ...
            # LeRobot v2 datasets may contain string provenance columns such as
            # source.split; these are not normalizable model modalities.
            continue
...
class LeRobotSingleDataset(Dataset):
    ...
    def _get_metadata(self, embodiment_tag: EmbodimentTag) -> DatasetMetadata:
        ...
        standard_stats_path = self.dataset_path / LE_ROBOT_STANDARD_STATS_FILENAME
        load_stats_path = stats_path if stats_path.exists() else standard_stats_path
        try:
            with open(load_stats_path, "r") as f:
            ...
            _atomic_write_json(stats_path, le_statistics)
        dataset_statistics = {}
    ...
    def _get_all_steps(self) -> list[tuple[int, int]]:
        ...
        try:
            if steps_path.exists():
                ...
                if cached_data.get("config_key") != config_key:
                    raise KeyError("Cached steps were created with a different configuration")
                if not cached_data["steps"] and np.sum(self.trajectory_lengths) > 0:
                    raise ValueError("Cached steps are empty for a non-empty dataset")
                return cached_data["steps"]
            raise FileNotFoundError(steps_path)
        except (FileNotFoundError, EOFError, pickle.PickleError, KeyError, ValueError) as e:
        ...
        try:
            ...
            _atomic_write_pickle(steps_path, cache_data)
    ...
    def get_language(
        ...
        for i in range(len(step_indices)):
            task_value = self.curr_traj_data[original_key].iloc[step_indices[i]]
            if isinstance(task_value, str):
                task_strings.append(task_value)
            else:
                task_indices.append(int(np.asarray(task_value).item()))
        if task_strings:
            if task_indices:
                raise ValueError(f"Mixed string and indexed tasks in annotation column {original_key}")
            return task_strings
