...
class RobotwinAgilexDataConfig:
...
class RealWidowXAIDataConfig:
    """Modality and normalization contract for the real bimanual WidowXAI data."""
    ...
    def transform(self):
        # WidowXAI grippers are continuous joint positions, so they must not use
        # the binary (> 0.5) normalization used by RoboTwin grippers.
        return ComposedModalityTransform(transforms=[
            StateActionToTensor(apply_to=self.state_keys),
            StateActionTransform(
                apply_to=self.state_keys,
...
ROBOT_TYPE_CONFIG_MAP = {
    ...
    "real_widowxai": RealWidowXAIDataConfig,
    "oxe_rt1": OxeRT1DataConfig,
    "robotwin": RobotwinAgilexDataConfig,
    "robotwin50": RobotwinAgilexDataConfig,
